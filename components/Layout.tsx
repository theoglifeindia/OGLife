...
import { ShoppingBag, Menu, X, Leaf, User, ArrowRight, Phone, Mail, MapPin } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans">
      <header className="bg-white/90 backdrop-blur-md sticky top-0 z-50 border-b border-gray-100 transition-all duration-300">
        ...
        <div className={`fixed inset-0 z-40 bg-brand-cream/95 backdrop-blur-xl transform transition-transform duration-500 ease-in-out md:hidden flex flex-col justify-center items-center ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
            <button className="absolute top-6 right-6 text-brand-dark p-2" onClick={closeMenu}><X className="h-8 w-8" /></button>
      ...
      <footer className="bg-brand-dark text-white pt-20 pb-10">
        <div className="max-w-7xl mx-auto px-6 lg:px-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8">
          <div className="col-span-1 md:col-span-2 lg:col-span-1">
            ...
            {info && (
              <div className="space-y-4 mt-8 border-t border-white/10 pt-6">
                <div className="flex items-start gap-4 text-sm text-gray-400 group">
                  <MapPin className="h-5 w-5 text-brand-gold shrink-0 mt-0.5" />
                  <span className="leading-relaxed group-hover:text-white transition">{info.address}</span>
                </div>
                <div className="flex items-center gap-4 text-sm text-gray-400 group">
                  <Phone className="h-5 w-5 text-brand-gold shrink-0" />
                  <span className="group-hover:text-white transition">{info.phone}</span>
                </div>
                <div className="flex items-center gap-4 text-sm text-gray-400 group">
                  <Mail className="h-5 w-5 text-brand-gold shrink-0" />
                  <span className="group-hover:text-white transition">{info.email}</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col">
            <h4 className="font-bold text-brand-gold uppercase tracking-[0.2em] text-xs mb-8">Certifications</h4>
            <div className="space-y-4 text-sm text-gray-400">
               <div className="flex flex-col">
                  <span className="text-brand-gold font-bold text-[10px] uppercase tracking-widest mb-1">FSSAI Lic. No</span>
                  <span className="bg-white/5 px-3 py-1 rounded w-fit font-mono">{info?.fssaiNo || 'PENDING'}</span>
               </div>
               <div className="flex flex-col">
                  <span className="text-brand-gold font-bold text-[10px] uppercase tracking-widest mb-1">GST Registration</span>
                  <span className="bg-white/5 px-3 py-1 rounded w-fit font-mono">{info?.gstNo || 'PENDING'}</span>
               </div>
            </div>
            <div className="mt-8">
              <Link to="/about" className="text-brand-gold font-bold uppercase text-xs tracking-widest flex items-center hover:text-white transition">
          ...
          <div className="col-span-1 md:col-span-2 lg:col-span-1">
            ...
            <div className="flex items-center gap-4">
               <span className="text-xs font-bold uppercase tracking-widest text-gray-500">Socials:</span>
               <a href="#" className="text-gray-400 hover:text-brand-gold transition text-sm">{info?.instagram || '@theoglife'}</a>
            </div>
          </div>
        </div>
